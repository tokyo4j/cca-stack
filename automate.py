...
PHASE_START_APP = 1
PHASE_LOOP = 2
phases = {"data": {0: PHASE_INIT, 1: PHASE_INIT}, "lock": threading.Lock()}
...
def handle_host(host_id, port):
    ...
    if host_id == 0:
        ...
        while True:
            with phases["lock"]:
                if phases["data"].get(0) >= PHASE_START_APP and phases["data"].get(1) >= PHASE_START_APP:
                    break
            sleep(child, 0.3)
        child.sendline("Started app")
        while True:
            with phases["lock"]:
                if phases["data"].get(0) >= PHASE_LOOP and phases["data"].get(1) >= PHASE_LOOP:
                    break
            sleep(child, 0.3)
        sleep(child, 3)
...
def handle_realm(realm_id, port):
    ...
    with phases["lock"]:
        phases["data"][realm_id] = PHASE_START_APP
    child.expect("Entering infinite loop...", timeout=None)
    with phases["lock"]:
        phases["data"][realm_id] = PHASE_LOOP
    child.expect(pexpect.EOF, timeout=None)
